...
import { IMAGE_PROCESSING } from "../constants.js";
export const IMAGE_CONFIG = {
  maxWidth: 1200,
  maxHeight: 16000,
  quality: 0.82,
  coverMaxWidth: 400,
  coverMaxHeight: IMAGE_PROCESSING.MAX_COVER_DIMENSION, 
  coverQuality: 0.85, 
};
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      const config =
        type === "cover"
          ? {
              maxW: IMAGE_CONFIG.coverMaxWidth,
              maxH: IMAGE_CONFIG.coverMaxHeight,
              quality: IMAGE_CONFIG.coverQuality,
            }
          : {
              maxW: IMAGE_CONFIG.maxWidth,
              maxH: IMAGE_CONFIG.maxHeight,
      ...
      if (height > config.maxH) {
        ...
        for (let i = 0; i < numParts; i++) {
          ...
          const base64 = canvas
            .toDataURL("image/webp", config.quality)
            .split(",")[1];
          parts.push({
            data: base64,
            suffix: `_part${i + 1}`,
