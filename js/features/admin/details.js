...
export async function loadSeriesDetail(name) {
  window.AdminActions?.switchView("series-detail");
    ...
    container.innerHTML = chapters
      .map((ch) => {
        ...
        return AdminChapterItem({
          ...
          onClickHandler: `AdminActions.openChapterDetail('${name}', '${ch}')`,
          onDeleteHandler: `event.stopPropagation(); AdminActions.deleteChapter('${name}', '${ch}', ${!!isArchived})`,
        });
      })
      .join("");
  }
...
export async function handleDeleteChapter(series, chapter, isArchived) {
  ...
  const deleteOp = async () => {
    if (isArchived) {
      ...
      const filesRes = await adminState.github.getContents(
        `content/${series}/${chapter}`,
      );
      if (!filesRes.ok) return Result.failure(filesRes.error);

      for (const f of filesRes.value) {
        if (f.type === "file")
          await adminState.github.deleteFile(f.path, f.sha);
      }
...
async function addToPendingDeletes(targetPath) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      const res = await adminState.github.request(
        `${API.CONTENT_BASE}/${FILE_PATH}`,
        {
          method: "PUT",
          body: JSON.stringify({
...
export async function loadChapterDetail(seriesName, chapterNum) {
  window.AdminActions?.switchView("chapter-detail");
  ...
  if (backLink) {
    backLink.onclick = (e) => {
      e.preventDefault();
      window.AdminActions?.openSeriesDetail(seriesName);
      return false;
    };
  }
  ...
  if (archived) {
    ...
    if (typeof archived === "object" && archived.mode === "count") {
      images = Array.from({ length: archived.data }, (_, i) => {
        ...
        return { name, download_url: encodeGitHubUrl(`${baseUrl}/${name}`) };
      });
    } else if (typeof archived === "object" && Array.isArray(archived.data)) {
      images = archived.data.map((name) => ({
        name: name,
        download_url: encodeGitHubUrl(`${baseUrl}/${name}`),
      }));
    } else {
      grid.innerHTML =
        '<div class="empty-state">Archived (Legacy Format)</div>';
      return;
    }
  } else {
    const encodedPath = `content/${encodeURIComponent(seriesName)}/${chapterNum}`;
    const contentsResult = await adminState.github.getContents(encodedPath);
    if (contentsResult.ok) {
      images = contentsResult.value.filter((f) =>
        f.name.match(/\.(webp|jpg|png)$/),
      );
      images = images.map((f) => ({
        name: f.name,
        path: f.path,
  ...
  if (images.length) {
    ...
    const imageElements = await Promise.all(
      images.map(async (img) => {
        let url = img.download_url ? encodeGitHubUrl(img.download_url) : "";

        if (img.path) {
          const blobResult = await adminState.github.fetchImageAsBlob(img.path);
          if (blobResult.ok) {
            url = blobResult.value;
          }
        }
        ...
        return AdminImageItem({
          ...
          onClick: "window.AdminActions.toggleImageSelection(this)",
        });
      }),
    );
    ...
    grid.innerHTML =
      '<div class="empty-state">Failed to load images or empty chapter.</div>';
  }
}
...
export async function deleteSelectedImages() {
  ...
  const contentsRes = await adminState.github.getContents(
    `content/${adminState.currentSeries}/${adminState.currentChapter}`,
  );
  if (!contentsRes.ok) return Toast.error("Failed to fetch contents");

  const toDelete = contentsRes.value.filter((f) =>
    adminState.selectedImages.has(f.name),
  );
  let deleted = 0;
...
export function setupDetailsHandlers(adminActions = window.AdminActions) {
  if (!adminActions) return;
  adminActions.toggleImageSelection = toggleImageSelection;
  adminActions.handleDeleteChapter = handleDeleteChapter;
}
...
export async function deleteSeries() {
  ...
  if (
    !confirm(
      `Delete "${currentSeries}"? All chapters and images will be permanently removed.`,
    )
  )
    return;

  const series = adminState.series.find((s) => s.name === currentSeries);
  const hasCloudChapters =
    series?.chapter_roots && Object.keys(series.chapter_roots).length > 0;
  ...
  if (hasCloudChapters) {
    ...
    if (result.ok) {
      ...
      await refreshSeriesData();
      window.AdminActions?.switchView("comic-series");
    } else {
      Toast.error("Delete failed");
    }
  } else {
...
export function changeCover() {
  window.AdminActions?.openModal("modal-change-cover");
}
...
export async function uploadNewCover() {
  ...
  const blobRes = await adminState.github.createBlob(parts[0].data, "base64");
  if (!blobRes.ok) return Toast.error("Failed to create blob for cover image");

  const commitRes = await adminState.github.atomicCommit(
    `Update cover: ${currentSeries}`,
    [
      {
...
export function uploadMoreImages() {
  window.AdminActions?.openModal("modal-upload-images");
...
export async function uploadModalImages() {
  ...
  const LOG_TYPE_ICON = Object.freeze({
    success: SUCCESS,
    error: ERROR,
    pending: PENDING,
  });
  ...
  const addLog = (msg, type = "pending") => {
    if (logDiv) {
      const iconText = LOG_TYPE_ICON[type] || LOG_TYPE_ICON.pending;
      ...
      logItem.append(iconWrap, " ", msgWrap);
  ...
  const task = async () => {
    ...
    addLog("Committing...");
    return await adminState.github.atomicCommit(
      `Add images to ${currentSeries} Ch${currentChapter}`,
      treeItems,
    );
  };
