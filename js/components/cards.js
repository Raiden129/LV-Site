...
export const Card = {
    series(series, onClick) {
        ...
        const cover = series.cover
            ? html`<img src="${encodedCover}" class="card-cover" loading="lazy" alt="${escapeAttr(series.title)}">`
    ...
    novel(series, onClick) {
        ...
        const sortedChapters = series.chapters?.length
            ? [...series.chapters].sort((a, b) => {
                const numA = typeof a === 'object' ? a.number : a;
                const numB = typeof b === 'object' ? b.number : b;
                return naturalSort(numA, numB);
            })
            : [];

        const latestChapterObj = sortedChapters.pop();
        const latestChapter = typeof latestChapterObj === 'object' ? latestChapterObj.number : latestChapterObj;

        const el = document.createElement('article');
        el.className = 'card-series card-novel';
        el.onclick = onClick;
        el.innerHTML = html`
            <div class="card-cover-wrapper">
                ${cover}
            ...
            <div class="card-info">
                <h3 class="card-title" title="${escapeAttr(series.title)}">${escapeHtml(series.title)}</h3>
                <div class="card-meta">
                    <span class="chapter-badge">
                         ${ICONS.SERIES_BADGE} Chapter ${latestChapter || '—'}
                    </span>
    ...
    chapter(chapter, seriesId, viewCount, onClick) {
        ...
        el.innerHTML = html`
            ...
            <div class="card-chapter-info">
                <span class="card-chapter-title">Chapter ${escapeHtml(chapter)}</span>
    ...
    empty(icon, title, subtitle = '') {
        ...
        el.innerHTML = html`
            ...
            ${subtitle ? html`<div class="empty-text">${escapeHtml(subtitle)}</div>` : ''}
        `;
        return el;
    }
};
...
export const AdminChapterItem = ({ chapter, seriesName, isArchived, onClickHandler, onDeleteHandler }) => {
    ...
    return html`
        <div class="chapter-item" onclick="${escapeAttr(onClickHandler)}">
            <div class="chapter-item-info">
                <div class="chapter-icon">${escapeHtml(chapter)}</div>
                <div class="chapter-title">Chapter ${escapeHtml(chapter)}</div>
                ${archivedTag}
            </div>
            <button class="btn-danger btn-sm" onclick="${escapeAttr(onDeleteHandler)}">Delete</button>
        </div>
    `;
};
...
export const AdminSeriesCard = (s, siteUrl) => {
    ...
    return html`
        <article class="card-series" onclick="AdminActions.openSeriesDetail('${escapeAttr(s.name)}')">
            <div class="card-cover-wrapper">
                ${coverHtml}
                <div class="card-cover-overlay"></div>
            </div>
...
export const AdminSeriesListItem = (s, siteUrl) => {
    ...
    return html`
        <div class="series-list-item" onclick="AdminActions.openSeriesDetail('${escapeAttr(s.name)}')">
            <div class="series-list-cover">
                ${coverHtml}
            </div>
            <div class="series-list-info">
                <div class="series-list-title">${escapeHtml(s.name)}</div>
